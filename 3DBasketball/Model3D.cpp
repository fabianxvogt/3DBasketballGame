...
namespace gps {
	...
	void Model3D::ReadOBJ(std::string fileName, std::string basePath){
		...
		std::string err, warn;
		bool ret = tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, fileName.c_str(), basePath.c_str(), GL_TRUE);
		...
		for (size_t s = 0; s < shapes.size(); s++) {
			...
			for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++) {
				...
				/*gps::Texture currentTexture = LoadTexture("basketball.mtl", "ambientTexture"); 
